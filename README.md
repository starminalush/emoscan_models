...
```
    ...
    │   └── data/transforms       <- Модуль аугментаций.
    │   └── models                <- Модуль c кастомными моделями.
    │   └── trainers              <- Модуль c трейнерами моделей(в них основной train loop и валидация моделей)
    |
    ├── data                      <- Папка, куда будут скачиваться данные и записываться данные после постпроцессинга.
    |
    ├── experiments               <- Модуль с экспериментами.
    │   └── configs               <- Конфиги обучения.
    │   └── converters            <- Конверторы моделей из формата в формат.
    │   └── data                  <- Модуль со скриптами для препроцессинга данных.
    │   └── dvc.yaml              <- Файл с описанием запуска пайплайна обучения с помощью dvc.
    │   └── train.py              <- Скрипт запуска обучения и инициализации нужных компонентов и dataloader'ов.
    |   └── utils.py              <- Полезные утилиты, нужные для обучения.
    |
    ├── models                    <- Папка для хранения обученных моделей.
    |
    ├── notebooks                 <- jupyter ноутбуки.
    |
    ├── references                <- Промежуточные отчеты и заметки от экспериментов.
    |
    ├── reports                   <- Auto-generated отчеты.
    |
    ├── submodules                <- Сабмодули с репозиториями обучений нейросетевых моделей, которые будут использоваться для дальнейших экспериментов
    |
    ├── .env.template             <- Шаблон .env файла.
    |
    ├── Dockerfile                <- Конфиг docker образа.
    |
    ├── Makefile                  <- Файл настройки проекта.
    |
    ├── docker-compose.yml        <- Compose file со всеми нужными для обучения сервисами.
    |
